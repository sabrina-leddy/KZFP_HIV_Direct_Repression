...
# Download raw fastqs for R1 and R2
...
printf "%s\n" "${donors[@]}" | parallel -j 15 '
  ...
  forward_reads="'"$input_dir"'/${donor}_1.fastq"
  reverse_reads="'"$input_dir"'/${donor}_2.fastq"

  echo "Forward reads: ${forward_reads}"
  echo "Reverse reads: ${reverse_reads}"

  output_forward_paired="'"$input_dir"'/${donor}_trimmed_1.fastq"
  output_reverse_paired="'"$input_dir"'/${donor}_trimmed_2.fastq"

  echo "Output forward paired: ${output_forward_paired}"
  echo "Output reverse paired: ${output_reverse_paired}"

  java -jar "${TRIM_JAR}" PE -phred33 \
    "${forward_reads}" "${reverse_reads}" \
    "${output_forward_paired}" "${output_forward_paired%.fastq}_unpaired.fastq" \
    "${output_reverse_paired}" "${output_reverse_paired%.fastq}_unpaired.fastq" \
...
for file in "$map_dir"/*_trimmed_1.bam; do
  # Extract the base name without extension or path
  base=$(basename "$file" "_trimmed_1.bam")
